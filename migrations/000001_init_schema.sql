...
CREATE TABLE `employees` (`id` VARCHAR(128) PRIMARY KEY UNIQUE ,`first_name` VARCHAR(64), `last_name` VARCHAR(64), `salary` INTEGER, `birthday` DATE, employee_number INTEGER UNIQUE, entry_date DATE);
CREATE TABLE `companycars` (`id` VARCHAR(128) PRIMARY KEY UNIQUE ,`manufacturer` VARCHAR(64), `type` VARCHAR(64), `number_plate` TEXT, employee_number INTEGER, FOREIGN KEY (employee_number) REFERENCES employees(employee_number));
